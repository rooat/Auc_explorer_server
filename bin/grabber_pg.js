...
class GrabberClass {
    ...
    async start(){
        ...
        let blockFind =await config.db.Block.findOne({}, "number").sort('-number');
        if(blockFind){
        ...
        await config.db.Block.collection.remove({'number':lastBlockNum+1});
        await config.db.LogEvent.collection.remove({'blockNumber':lastBlockNum+1});
        await config.db.Contract.collection.remove({'blockNumber':lastBlockNum+1});
        await config.db.TokenTransfer.collection.remove({'blockNumber':lastBlockNum+1});
        await config.db.InerTransaction.collection.remove({'blockNumber':lastBlockNum+1});
        ...
        setInterval( async ()=>{
            ...
            if(lastBlockNum < newBlockNumber-delayBlock){
                lastBlockNum++;
                this.grabBlock( lastBlockNum);
            }
        },1000);
    }
    async grabBlock(desiredBlockHashOrNumber) {
    ...
    async writeBlockToDB(blockData) {
        ...
        if(!block){
            ...
            let version;
            let extraData = this.hex2ascii(blockData.extraData);
    ...
    async upsertAddress(miner, addrs){
        if(miner){
            //add reward to master node
            await config.db.Address.update({"addr":miner},
        ...
        if(addrs){
            for(let i=0; i<addrs.length; i+=2){
                let doc = await config.db.Address.update({"addr":addrs[i]},
                    // {$set:{"balance":balance}},
            ...
            for (d in blockData.transactions) {
                ...
                if(txData.input && txData.input.length>2){// contract create, Event logs of internal transaction
                    if(txData.to == null){//contract create
                        ...
                        var Token = new config.web3.eth.Contract(ERC20ABI,receiptData.contractAddress);
